...
import { BUILTIN_ARITY } from '../keymap-dt/lint'
import {
  BEHAVIORS,
  KEYCODES,
  applyModifier,
...
describe('picker dictionaries', () => {
  ...
  it('the picker covers every behaviour the lint table knows', () => {
    // Every static builtin the lint recognises must be pickable — otherwise a
    ...
    for (const token of Object.keys(BUILTIN_ARITY)) {
