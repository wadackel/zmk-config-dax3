...
// Per-row matrix column occupancy — parallels dax3.dtsi's default_transform.
// (row, col) tuples in physical index order. See dax3.dtsi and matrix-mapping.ts.
...
export function serializeBindingsGrid(bindings: BindingChain[]): string {
  if (bindings.length !== DAX3_KEY_COUNT) {
    throw new Error(
      `serializeBindingsGrid: expected ${DAX3_KEY_COUNT} bindings, got ${bindings.length}`,
    )
  }
