...
/**
 * Behaviour arity table. Built-in ZMK behaviours plus dax3 customs. Keys are
 * the behaviour name WITH leading `&`. Value lists are the allowed argument
 * counts (some behaviours accept multiple arities; e.g. macro shorthands).
 *
 * Picker (Task 12) provides a richer source of truth; this table is the
 * minimum the lint pass needs to operate, and overlaps with the picker entries.
 */
export const BUILTIN_ARITY: Record<string, number[]> = {
  ...
  '&inc_dec_kp': [2],
  // zmk-mouse-gesture (v1) — pinned via config/west.yml.
  '&mouse_gesture': [0],
  '&mouse_gesture_toggle': [0],
  '&mouse_gesture_on': [0],
  '&mouse_gesture_off': [0],
  '&mouse_gesture_kp': [2],
  '&mouse_gesture_mkp': [2],
...
// esc_lang2_with_layer, ...) get added at lint time.
...
export function lint(source: string): LintResult {
  ...
  for (const layer of parsed.layers) {
    if (layer.bindings.length !== DAX3_KEY_COUNT) {
      errors.push({
        severity: 'error',
        message: `layer ${layer.name}: expected ${DAX3_KEY_COUNT} bindings, got ${layer.bindings.length}`,
      })
    }
  }

  // Build the set of known behaviour names: builtins + custom behaviours + macros.
  const arityTable: Record<string, number[]> = { ...BUILTIN_ARITY }
  ...
  // 4. Combo: key-positions in 0..DAX3_KEY_COUNT-1 + bindings reference exists + layers in range.
  const maxKeyIdx = DAX3_KEY_COUNT - 1
